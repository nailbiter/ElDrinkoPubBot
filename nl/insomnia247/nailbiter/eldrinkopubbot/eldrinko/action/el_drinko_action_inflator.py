...
from nl.insomnia247.nailbiter.eldrinkopubbot.util import parse_ukrainian_float
...
#                        bottles.put(bottleType,bottles.getInt(bottleType) + (shouldAdd?1:-1));
                bottles[bottleType] = bottles[bottleType] + (1 if shouldAdd else -1)
...
#                        im.right.put("payment", paymentMethods.split("\n")[i]);
                tka = im.input_message
                i = int(tka.message)
                paymentMethods = self._jinja_env.get_template("4ea9a63509e8ed5826a37f8a.txt").render().split("\n")
...
#                    _orderInserter.accept(new Document(map));
            map_ = ElDrinkoActionInflator._order_object_to_jinja_context(order,im.beerlist)
...
#                                ,"salesmanChatIds"));
            self._sendOrderCallback(self._jinja_env.get_template("3804e512b18b339fe8786dbd.txt").render())
#                    _sendOrderCallback.accept(new ImmutablePair<String,String>(
#                            MiscUtils.ProcessTemplate("3804e512b18b339fe8786dbd",map,im.beerlist)
